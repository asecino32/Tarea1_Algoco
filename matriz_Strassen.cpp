...
// suma las dos matrices y retora el valor de la suma
...
// sacado de: https://www.naukri.com/code360/library/strassens-matrix-multiplication
/*
    Recibe dos matrices A y B, verifica si las matrices se pueden multiplicar,
    inicializa la matriz de resultado llena de ceros, calcula el indice para dividir las 
    matrices en submatrices, luego calcula los productos parciales p1 a p7, utilizando
    la multiplicación recursiva calcula las submatrices del resultado combinando los
    productos parciales, luego une las submatrices resultantes en la matriz de resultado
    la cual retorna.
*/
vector<vector<int>> strassen_multiplication(vector<vector<int>> A, vector<vector<int>> B) {
    int col_1 = A[0].size(); // Número de columnas de A
    int row_1 = A.size();     // Número de filas de A
    int col_2 = B[0].size();  // Número de columnas de B
    int row_2 = B.size();     // Número de filas de B
    if (col_1 != row_2) {
